		...
		if($query->num_rows > 0){
			...
			if($status == 'in'){
				...
				if($query->num_rows > 0){
					...
					//
					$sql = "INSERT INTO attendance (employee_id, date, time_in, status,time_out,num_hr) VALUES ('$id', '$date_now', NOW(), '$logstatus','00:00:00','8')";
					if($conn->query($sql)){
				...
				$sql = "SELECT *, attendance.id AS uid FROM attendance LEFT JOIN employee ON employee.id=attendance.employee_id WHERE attendance.employee_id = '$id' AND date = '$date_now'";
				$query = $conn->query($sql);
				if($query->num_rows < 1){
					$output['error'] = true;
